...
const showSubjectGrades = (subject, subjectGrades) => {
    ...
    const subjectGrade = subjectGrades[5];

    subjectsRevealedState[subjectCode] = true;
    updateLocalStorage();

    for (let j = GRADES_START_COLUMN; j < GRADES_END_COLUMN; j++) {
        subjectGradesDOM[j].style.removeProperty('background-color');
        subjectGradesDOM[j].innerHTML = subjectGrades[j];
    }

    const gradeIndex = gradesLetters.indexOf(subjectGrade);
    for (let j = 0; j < subjectGrades.length; j++) {
        subjectGradesDOM[j].style.backgroundColor = gradesColors[gradeIndex];
    }
